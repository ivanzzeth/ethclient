...
func TestSendTransaction(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		testSendTransaction(t, sim)
	}
}

func testSendTransaction(t *testing.T, sim *Backend) {
	client := sim.Client()
	...
	if err != nil {
		t.Errorf("could not get block at hash %v: %v", blockHash.Hex(), err)
	}

	if signedTx.Hash() != block.Transactions()[0].Hash() {
		t.Errorf("did not commit sent transaction. expected hash %v got hash %v", signedTx.Hash(), block.Transactions()[0].Hash())
	}

	_, err = client.TransactionReceipt(ctx, signedTx.Hash())
	if err != nil {
		t.Fatalf("could not get receipt at hash %v: %v", signedTx.Hash().Hex(), err)
	}
}
